# Implementation checklist

- [x] Milestone 1: app foundation, localization, onboarding, auth boundary and home shell
- [ ] Milestone 2: Firestore catalog, search, locations and price resolver
- [ ] Milestone 3: cart, checkout, idempotent order creation and history
- [ ] Milestone 4: secure admin dashboard and management workflows
- [ ] Milestone 5: FCM and trusted Excel/Graph synchronization
- [ ] Milestone 6: rules, offline/performance QA, release builds and store assets

